...
import static until.FlorenceUntil.showListBeforeRecursion;
...
public class Week4 {
    ...
    public static <T> void deleteAfterNodeA(Node<T> head, Node<T> NodeA) {
        Node<T> cur = head;
        while (cur.next != null && cur.data != NodeA.data) {
            cur = cur.next;
        }
        //到达最后面的点
        if (cur.next == null) {
    ...
    public static <T> void deleteBeforeNodeA(Node<T> head, Node<T> NodeA) {
        Node<T> cur = head;
        while (cur.next != null && cur.next.next != null && cur.next.next.data != NodeA.data) {
            cur = cur.next;
        }
